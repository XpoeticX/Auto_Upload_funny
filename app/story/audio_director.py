...
def synthesize_character_voice(text: str, output_path: str, voice: str = "en-US-AnaNeural") -> Optional[str]:
    """
    Synthesizes cartoon character dialogue using edge-tts with pitch/speed modulation.
    Guarantees the character is never mute.
    """
    try:
        ...
        if res.returncode == 0 and os.path.exists(output_path) and os.path.getsize(output_path) > 1000:
            print(f"[AUDIO DIRECTOR] Synthesized character voice: '{clean_text}' -> {output_path}")
            return output_path
    except Exception as e:
        print(f"[AUDIO DIRECTOR] Voice synth notice: {e}")
    return None
...
def build_scene_audio_timeline(story: Dict, total_duration: float = 8.6, output_wav: str = "data/temp/master_studio_audio.wav") -> str:
    ...
    - Identifies each scene's timing, character voice lines, and Foley action.
    - Synthesizes character speech so the protagonist is NEVER mute.
    - Curates Foley cues (MAX 3 total, spaced >= 2.5s) to eliminate noise clutter.
    - Ducks background music under character voice and impact punches.
    """
    scenes = story.get("scenes", [])
    audio_cfg = story.get("audio_config", {})
    music_vibe = audio_cfg.get("bgm_style", story.get("music_vibe", "bouncy_comedy"))
    bgm_vol = audio_cfg.get("bgm_base_volume", 0.55)
    ...
    filter_parts = []
    mix_inputs = ["[bgm]"]
    # Energetic background music bed
    filter_parts.append(f"[0:a]volume={bgm_vol},atrim=0:{total_duration},asetpts=PTS-STARTPTS[bgm]")
    input_idx = 1

    # --- 1. CHARACTER VOICE TRACKS (Character speech is top priority) ---
    current_time = 0.0
    for i, sc in enumerate(scenes):
        sc_dur = float(sc.get("duration_sec", 2.5))
        voice_text = sc.get("dialogue") or sc.get("voice_line") or sc.get("character_voice")
        if voice_text:
            voice_out = os.path.join("data", "temp", f"character_voice_act_{i+1}.mp3")
            v_path = synthesize_character_voice(voice_text, voice_out)
            if v_path and os.path.exists(v_path):
                inputs.extend(["-i", v_path])
                voice_delay_ms = int((current_time + 0.3) * 1000)
                v_label = f"voice{input_idx}"
                # Prominent character voice (volume=1.8)
                filter_parts.append(
                    f"[{input_idx}:a]volume=1.8,asetpts=PTS-STARTPTS,adelay={voice_delay_ms}|{voice_delay_ms}[{v_label}]"
                )
                mix_inputs.append(f"[{v_label}]")
                input_idx += 1
        current_time += sc_dur

    # --- 2. FOLEY SOUND EFFECTS (Curated & strictly capped) ---
    ...
    if has_scene_foley_cues:
        ...
        for sc in scenes:
            ...
            for cue in sc.get("foley_cues", []):
                ...
                vol = min(float(cue.get("volume", 1.2)), 1.3)
        ...
        print(f"[AUDIO DIRECTOR] Selected {len(selected_cues)} clean Foley cues (no clutter)")
        ...
        # Default scene timing fallback (only when no foley_cues are defined)
        ...
        for i, sc in enumerate(scenes[:2]):
            ...
            label = f"sfx{i+1}"
            filter_parts.append(
                f"[{input_idx}:a]volume=1.2,atrim=0:{dur},asetpts=PTS-STARTPTS,adelay={delay_ms}|{delay_ms}[{label}]"
            )
            mix_inputs.append(f"[{label}]")
            input_idx += 1
